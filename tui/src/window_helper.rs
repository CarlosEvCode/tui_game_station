...
pub fn minimize_active_window() {
    ...
    let is_hyprland =
        desktop.contains("hyprland") || env::var("HYPRLAND_INSTANCE_SIGNATURE").is_ok();
    let is_sway = desktop.contains("sway") || env::var("SWAYSOCK").is_ok();

    if is_mango {
        let _ = Command::new("mmsg")
            .args(["dispatch", "minimized"])
            .output();
    } else if is_hyprland {
        ...
        let _ = Command::new("swaymsg")
            .args(["move", "scratchpad"])
            .output();
    } else if desktop.contains("niri") {
        let _ = Command::new("niri")
            .args(["msg", "action", "move-column-to-workspace-down"])
            .output();
    } else {
...
pub fn restore_active_window() {
    ...
    if is_mango {
        let _ = Command::new("mmsg")
            .args(["dispatch", "restore_minimized"])
            .output();
    } else if is_hyprland {
        let _ = Command::new("hyprctl")
            .args(["dispatch", "movetoworkspace", "e+0"])
            .output();
    } else if is_sway {
        let _ = Command::new("swaymsg")
        ...
        let _ = Command::new("niri")
            .args(["msg", "action", "move-column-to-workspace-up"])
            .output();
    } else {
...
pub fn set_fullscreen(enable: bool) {
    ...
    if is_mango {
        let _ = Command::new("mmsg")
            .args(["dispatch", "togglefullscreen"])
            .output();
    } else if is_hyprland {
        let mode = if enable { "1" } else { "0" };
        let _ = Command::new("hyprctl")
            .args(["dispatch", "fullscreen", mode])
            .output();
    } else if is_sway {
